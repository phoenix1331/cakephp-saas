...
<div class="mt-2 flex items-start justify-between">
    <div>
        <h1 class="text-2xl font-semibold text-gray-900"><?= h($service->name) ?></h1>
        <p class="mt-1 text-sm text-gray-500">
            <?= $this->Number->format($service->duration_minutes) ?> <?= __('minutes') ?>
            &middot;
            <?= $this->Number->currency($service->price, 'GBP') ?>
        </p>
    </div>
    <div class="flex gap-2">
        <?= $this->Html->link(__('Edit'), ['action' => 'edit', $service->id], ['class' => 'rounded-lg border border-gray-300 px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50']) ?>
        <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $service->id], ['confirm' => __('Are you sure you want to delete # {0}?', $service->id), 'class' => 'rounded-lg border border-red-200 px-3 py-2 text-sm font-medium text-red-600 hover:bg-red-50']) ?>
    </div>
</div>

<div class="mt-8">
    <h2 class="text-sm font-medium text-gray-900"><?= __('Staff who can perform this service') ?></h2>
    ...
    <?php else : ?>
    <p class="mt-3 text-sm text-gray-500"><?= __('No staff assigned to this service yet.') ?></p>
    <?php endif; ?>
</div>

<div class="mt-8">
    <h2 class="text-sm font-medium text-gray-900"><?= __('Bookings') ?></h2>
    <?php if (!empty($service->bookings)) : ?>
    <div class="mt-3 overflow-hidden rounded-lg border border-gray-200 bg-white">
        <table class="min-w-full divide-y divide-gray-200 text-sm">
            <thead class="bg-gray-50">
                <tr>
            ...
            <tbody class="divide-y divide-gray-100">
                <?php foreach ($service->bookings as $booking) : ?>
                <tr>
    ...
    <?php else : ?>
    <p class="mt-3 text-sm text-gray-500"><?= __('No bookings for this service yet.') ?></p>
    <?php endif; ?>
</div>
